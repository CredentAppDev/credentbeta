...
const startBackgroundJobs = () => {
  // screen_session_signals grows by ~10-40 rows per session and the table is
  // never read for historical purposes. Purge once after DB init and every
  // hour after that so it stays bounded.
  purgeStaleSignals()
...
const startServer = () => {
  ...
  initializeDatabase().catch((error) => {
    ...
    console.error('Server is still running in degraded mode. Fix DATABASE_URL or database quota to restore full API functionality.');
  });
};
