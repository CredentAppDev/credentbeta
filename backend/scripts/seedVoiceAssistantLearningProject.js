...
const chunks = [
  {
    ...
    source_type: 'lesson',
    audience: 'both',
    tags: ['overview', 'voice', 'assistant', 'esp32', 'xiaozhi', 'mcp'],
    content: [
      'This Class 6 project teaches learners to build a voice-controlled AI assistant using the ESP32-S3 AI Cam, Xiaozhi, and MCP.',
  ...
  {
    step_number: 9,
    title: 'Xiaozhi Wi-Fi and device configuration',
    source_type: 'setup',
    audience: 'both',
    tags: ['xiaozhi', 'wifi', 'pairing', 'console', 'configuration'],
    content: [
      'After powering on Mino, connect to the Xiaozhi hotspot from a phone or computer.',
      'Open a browser and go to 192.168.1.4, then enter the Wi-Fi details.',
      'The device gives a six-digit pairing code. Use the Xiaozhi console at xiaozhi.me to add the device and configure its role.',
      'Students should not skip the pairing step because the backend/cloud role setup controls how the assistant behaves.',
    ].join(' '),
  },
  {
    step_number: 10,
  ...
  },
  {
    ...
    content: [
      ...
      'This microphone is what Xiaozhi uses to capture voice commands, so these pins must be initialized correctly before voice interaction will work.',
    ].join(' '),
  },
  {
    step_number: 13,
  ...
  },
  {
    ...
    content: [
      ...
      'If the response is missing or shows an error, the teacher should first check that the board is powered, connected to Wi-Fi, and paired with Xiaozhi before assuming the code is wrong.',
    ].join(' '),
  },
  {
    step_number: 16,
    ...
    content: [
      ...
      'Wi-Fi setup failure: if the board does not broadcast the Xiaozhi hotspot after power on, press the reset button once and wait 10 seconds before trying again.',
      ...
      'MCP tool not responding: confirm the device role is correctly set in the xiaozhi.me console and that the tool name in the JSON matches exactly what the role configuration expects, including letter case.',
    ].join(' '),
  },
];
...
const roadmapDays = [
  ...
  },
  {
    ...
    next_day_prep: 'Prepare phone/computer Wi-Fi access, Xiaozhi hotspot instructions, and xiaozhi.me console access.',
  },
  {
    day_number: 4,
    title: 'Xiaozhi setup, pairing, and MCP tool/code explanation',
    teacher_goal: 'Connect the device to Wi-Fi, pair it with Xiaozhi, and explain MCP tools before learners use any code.',
    student_goal: 'Students can follow the pairing flow and explain what a tool command is supposed to do before running it.',
    activities: [
      'Power on Mino and connect to the Xiaozhi hotspot.',
      'Open 192.168.1.4 and enter Wi-Fi details.',
      'Collect the six-digit pairing code.',
      'Add the device in xiaozhi.me and configure its role.',
      'Explain one MCP JSON/tool example by purpose, input fields, hardware action, and expected response.',
    ],
    materials: ['Mino device', 'phone or computer', 'Wi-Fi details', 'xiaozhi.me console', 'project code/MCP example'],
    ...
    end_of_day_checklist: [
      'Device connects to Xiaozhi hotspot',
      'Wi-Fi credentials entered through 192.168.1.4',
      'Pairing code received',
      'Device added in xiaozhi.me console',
...
const seed = async () => {
  ...
  const project = await createLearningProject({
    ...
    grade: 'Class 6',
    class_name: 'Class 6',
    description: 'A Credent Class 6 project for building a voice-controlled assistant with ESP32-S3 AI Cam, Xiaozhi, MCP, speaker output, battery power, and a 3D printed enclosure.',
    learning_goals: 'Understand MCP as a safe bridge for AI tools, identify hardware parts, wire the BMS/switch/speaker correctly, configure Xiaozhi, and explain every code or MCP step before using it.',
